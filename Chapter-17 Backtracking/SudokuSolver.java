public class SudokuSolver {
    // Method to check if placing a digit is safe
    ...
    public static boolean sudokuSolver(int sudoku[][], int row, int col) {
        // Base case: if we have filled all rows, the puzzle is solved
        if (row == 9) {
            return true;
        }
        ...
        for (int digit = 1; digit <= 9; digit++) {
            if (isSafe(sudoku, row, col, digit)) {
                ...
                if (sudokuSolver(sudoku, nextRow, nextCol)) {
                    return true;  // If placing the digit leads to a solution, return true
                }
                sudoku[row][col] = 0;  // Backtrack by resetting the cell
            }
        }

        return false;  // If no digit works, return false
    }
